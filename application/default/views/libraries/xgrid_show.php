        ...
        <tbody>
            ...
            <?php else: ?>
                <?php foreach ($data as $row): ?>
                    <?php $stat = array_search('status', $self->fields);?>
                    <?php if(empty($stat)):?>
                                ...
                                <td class="submenu">
                                    <div class="submenu-container">
                                        <?php foreach ($self->actions as $key => $action): ?>
                                            ...
                                            <span class="<?php echo $key ?>"><a class="grid-action" href="<?php echo site_url($action . '/' . $row['id']) ?>" title="<?php echo l(humanize($key)) ?>"></a></span>
                                            <?php endif ?>
                                        <?php endforeach ?>
                                    </div>
                                </td>
                    ...
                    <?php else:?>
                        <tr data-ref="<?php echo (isset($row['id'])) ? $row['id'] : '' ?>" class="grid_row">
                            ...
                            <?php endif ?>
                            <?php for ($i = 0; $i < count($self->fields); $i++): ?>
                                <td <?php echo (empty($self->aligns[$i])) ? '' : 'style="text-align:' . $self->aligns[$i] . '"' ?> <?php if($row['status'] == 1):?> style="font-weight:bold" <?php endif;?> class="<?php echo (empty($self->classes[$i])) ? '' : $self->classes[$i] ?>">
                                    <span><?php echo $self->highlight($row, $i, ((!empty($filter['q'])) ? $filter['q'] : '')) ?></span>
                                </td>
                            <?php endfor ?>
                            <?php if (!empty($self->actions)): ?>
